...
public class ObservationCamera : MonoBehaviour {
    ...
    public float minCameraAngleX = 345.0f; // カメラの最小角度
    ...
    public float Zoom_min = -0.75f;  //キャラクターとの最小距離
    public float Zoom_max = 1.0f; //キャラクターとの最大距離
        ...
        */
		...
		if ((Input.touchCount == 1 && !isMouseDown)|| Input.GetMouseButtonDown(0)) {
            ...
            if (Input.touches[0].phase == TouchPhase.Ended || Input.touches[1].phase == TouchPhase.Ended)
            {
                ...
                float pinchZoomDistance = (basePinchDistance - currentPinchDistance) * pinchZoomSpeed * 0.01f;
                float cameraPosZ = baseCameraPos.z - pinchZoomDistance;
                if(cameraPosZ> Zoom_max)
                {
                    cameraPosZ = Zoom_max;
                }
                if (cameraPosZ< Zoom_min)
                {
                    cameraPosZ = Zoom_min;
                }
